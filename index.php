...
if (!empty($searchTerm)) {
    $query = urlencode('(' . $searchTerm . ') AND (game OR gaming OR videogame)');
} else {
    // Your custom default query for the feed
    $query = urlencode("gaming OR \"video games\" OR esports OR \"game releases\"");
}
$endpoint = "https://newsapi.org/v2/everything?apiKey={$apiKey}&q={$query}&language=en&pageSize=10&sortBy=relevancy";
$gamingKeywords = [
'game','gaming','videogame','playstation','xbox','steam','nintendo','pc','dlc','console','fighterz','xenoverse','sparking zero','kakarot'
];
...
function calculateRelevanceScore($article, $searchTerms) {
    ...
    global $gamingKeywords;
    foreach ($gamingKeywords as $keyword) {
        $score += substr_count($title,       $keyword) * 3;
        $score += substr_count($description, $keyword) * 1;
    }

    return $score;
}
function isGamingArticle($article) {
    global $gamingKeywords;
    $text = strtolower(
        ($article['title'] ?? '') . ' ' .
        ($article['description'] ?? '')
...
if (!empty($searchTerm) && isset($newsData['articles'])) {
    $newsData['articles'] = array_values(
        array_filter(
            $newsData['articles'],
            'isGamingArticle'
        )
...
<body>
    <header>
        <h1><a href="index.php"  style="text-decoration: none; color: inherit;">🎮 News 4 gamers</a></h1>
        <p><b>Latest updates on gaming. Find content that interests and excites you!</b></p>
    </header>
...
<div class="news-list">
    <?php
        $articlesCount = isset($newsData['articles']) && is_array($newsData['articles']) ? count($newsData['articles']) : 0;
    ?>
...
Relevance Rank: <?php echo isset($news['relevance_score']) ? (int) round($news['relevance_score']) : 'N/A'; ?>
                        </span>
                        <?php endif; ?>
                    </div> 
